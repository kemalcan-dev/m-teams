...
const backendOutput = {
    ...
    MONGO_DATABASE: 'm-teams',
    MAILER_ENABLED: false,
};

let frontendOutput = {
    VITE_SITE_TITLE: 'm-teams',
    VITE_BACKEND_URL: 'http://localhost',
    VITE_DEMO: false,
    VITE_SITE_BRAND: 'm-teams',
...
(async () => {
    ...
    console.log("");
    console.log("m-teams".yellow);
    console.log(`m-teams v${pkg.version} Installer`.yellow);
    console.log("");
    ...
    if (arg === 'setup') {
        ...
        console.log('m-teams needs to know the public ip address of your machine.'.cyan);
        ...
        console.log('');
        console.log('m-teams requires a domain, such as elderberry.example.com.'.cyan);
    ...
    if (['setup', 'rebuild'].includes(arg)) {
        ...
        if (!os.includes('Ubuntu')) {
            ...
            console.log('If you are running this on Ubuntu, please contact m-teams Support.');
        ...
        if (!version.includes('22.04') && !version.includes('20.04') && !version.includes('18.04')) {
            ...
            console.log('If you are running this on a correct version, please contact m-teams Support.');
            return process.exit(0);
        }
        ...
        console.log(`${arg === 'setup' ? 'installing' : 'rebuilding'} m-teams backend...`.yellow);
        ...
        try {
            exec('pm2 delete --silent m-teams', {stdio : 'pipe'});
        } catch (e) {}
        exec('cd ../backend && pm2 start index.js --name m-teams');
        exec('pm2 save');
        exec('pm2 startup');
        console.log('m-teams backend started'.green);

        console.log('');

        console.log(`${arg === 'setup' ? 'installing' : 'rebuilding'} m-teams frontend...`.yellow);
        ...
        console.log('m-teams frontend ok'.green);
    }
    ...
    if (arg === 'restart') {
        try {
            exec('pm2 restart m-teams', {stdio : 'pipe'});
        } catch (e) {}
    }

    if (arg === 'stop') {
        try {
            exec('pm2 stop m-teams', {stdio : 'pipe'});
        } catch (e) {}
    }
    ...
    if (arg === 'stop') {
        console.log("");
        console.log(`m-teams has been stopped.`.green);
        console.log("");
    } else {
        console.log("");
        console.log(`m-teams v${pkg.version} ${arg === 'setup' ? 'setup' : 'restart'} complete!`.green);
        if (arg === 'setup') {
            console.log(`You should now be able to access m-teams at ${domain}`);
        }
        console.log("");
    }
})();
